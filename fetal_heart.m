...
t = 0:1/fsample:(length(Y_left_filtered)-1)/fsample;
env = envelope(Y_left_filtered,150,'analytic');
[peaks, location] = findpeaks(env, 'MinPeakProminence', 0.01);

% Plot the signal power over time
figure(figure_num)
figure_num = figure_num + 1;
% plot(t,Y_left_filtered)
% hold on;
plot(t,env)
xlabel('Time (s)');
ylabel('Signal Power dB/Hz');
...
% Overlay the scatterplot of peaks
hold on;
% scatter((location-1) / fsample, peaks);
